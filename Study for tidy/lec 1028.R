...
delay <- filter(delay, count > 20, dest != "HNL")   # 비행건수 20 이상인 
...
# 아래와 같이 계산해도 같은 결과
# dep_delay, arr_delay 가 missing value 가 아닌 데이터 추출
...
# delay 분포 살펴보기
# 비행기별(tail_num)로 도착딜레이의 평균을 구하기
delays <- not_cancelled %>% 
  group_by(tail_num) %>%              # 비행기별
...
not_cancelled %>% filter(arr_delay > 0) %>% select(year:day, arr_delay)
...
not_cancelled %>% group_by(year, month, day) %>%  # 날짜별로
  mutate(r1 = min_rank(desc(dep_time)),
...
not_cancelled %>% group_by(year, month, day) %>%  # 날짜별로
  mutate(r = min_rank(desc(dep_time))) %>%        # 순위매겨서
...
not_cancelled %>% count(tail_num)                 # 비행기별 운항건수
not_cancelled %>% count(tail_num, wt = distance)  # 비행기별 누적 운항 거리
not_cancelled %>% count(tail_num, wt = distance) %>% arrange(desc(n))  # 내림차순
not_cancelled %>% count(tail_num, wt = distance) %>% arrange(n)  # 오름차순

flights %>% group_by(tail_num) %>% 
  summarise(sum(distance)) %>% arrange(desc(`sum(distance)`))
 
# dplyr::count

# sum(x > 10) : 횟수   sum 안에 조건문 - 합계  (TRUE개수)
# mean(y == 0) : 비율. mean 안에 조건문 - 비율 (TRUE/(TRUE+FALSE개수))
...
# year, month [?]
(per_day <- summarise(daily,            # 날짜별로
                     flights = n()))    # 운항건수를 flights 변수에 저장

# year [?]
(per_month <- summarise(per_day,                 # 그룹이? 
